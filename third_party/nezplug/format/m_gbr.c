...
#if 0
	/*
		TEKKA.GBR(TEKKAMAN BLADE)対暉 (PLAYからのRET前に余分なPOP) Ver.1(廃止)
		  こちらの方がわかりやすい扎Wiz外伝で異晨截
	*/
...
#else
	/*
		TEKKA.GBR対暉 (PLAYからのRET前に余分なPOP) Ver.2
		  割り撼み禁止(Wiz外伝対暉)
	*/
...
#if GBS2GB_EMULATION
	/*
		敍攜アドレスに飛んだときのレジスタの値を戡GBS2GBに插わせるテスト
	*/
...
static Uint32 load(NEZ_PLAY *pNezPlay, GBRDMG *THIS_, Uint8 *pData, Uint32 uSize)
{
	...
	if (THIS_->isgbr)
	{
		...
		//ヘッダ0FHの敘昴位ビットが立っていて戡その昴位４ビットが全部立っていないならGBC
		//昴位４ビットが全部立っている = タイマ拏未使用ビットを全部立たせてる = GBCとは限らない
...
First TMC          : %02XH"
	...
	else
	{
		...
		//なんか抓配な時用截昴位４ビットが全部立っていないなら有効としておく截拱とき�揀ｕﾓりとか挧
		//THIS_->useINT = ((pData[0x0f] & 0x44) == 0x44 && (pData[0x0f] & 0xf0) != 0xf0)? 1 : 0;

		//言われるがままにソ拏スを戮攀したが戡一部のGBSが敍攜されなくなったのはこの部分のせい
		THIS_->useINT = ((pData[0x0f] & 0x44) == 0x44 )? 1 : 0;
		{
			extern int gc_nez_gbs_force_int;
			if (gc_nez_gbs_force_int)
				THIS_->useINT = 1;
		}
...
Use INT           : %d"
	...
	/*
		MARIO2.GBR(SUPER MARIO LAND)対暉 (ファイルサイズ挂擣のため#0と#1を交換)
	*/
	if (THIS_->bankromnum == 2 && THIS_->bankromfirst[0] == 1 && THIS_->bankromfirst[1] == 0)
	{
		/* 4000-7FFFには#0は選択できないので本来は不攀 */
