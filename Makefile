...
#   /usr/bin/make pack     # /workspace/xmp-gamemusic-1.0.8.zip
#
...
pack: dll
	rm -f /workspace/xmp-gamemusic-1.0.8.zip
	mkdir -p $(DIST)/pack
	cp -f $(DIST)/xmp-gamemusic.dll $(ROOT)/README.md $(DIST)/pack/
	cd $(DIST)/pack && (command -v zip >/dev/null && zip -9 /workspace/xmp-gamemusic-1.0.8.zip xmp-gamemusic.dll README.md || python3 -c "import zipfile; z=zipfile.ZipFile('/workspace/xmp-gamemusic-1.0.8.zip','w',zipfile.ZIP_DEFLATED); z.write('xmp-gamemusic.dll'); z.write('README.md'); z.close()")
	rm -rf $(DIST)/pack
	ls -l /workspace/xmp-gamemusic-1.0.8.zip
	sha256sum /workspace/xmp-gamemusic-1.0.8.zip
